    ...
    const keyboard = {
        inline_keyboard: [
            ...
            [
                ...
                { text: '💬 Active Chats', callback_data: 'admin_chats' }
            ],
            [
                { text: '📢 Broadcast All', callback_data: 'admin_broadcast' },
                { text: '⭐ Broadcast Premium', callback_data: 'admin_broadcast_premium' }
            ]
        ]
    };

    const adminHelp = `🛡️ *Admin Panel*\n\n` +
        `*User Management:*\n` +
        `/ban <id> [reason] - Ban user\n` +
        `/unban <id> - Unban user\n` +
        `/resetreports <id> - Reset reports\n\n` +
        `*Premium Management:*\n` +
        `/grantpremium <id> <days> - Grant premium\n` +
        `/cancelpremium <id> - Cancel premium\n\n` +
        `*Chat Management:*\n` +
        `/activechats - View active chats\n` +
        `/endchat <id> - End user's chat\n\n` +
        `*Broadcast:*\n` +
        `/broadcast - Broadcast to all\n` +
        `/broadcastpremium - Premium only\n\n` +
        `*Info Commands:*\n` +
...
bot.on('callback_query', async (query) => {
    ...
    if (data.startsWith('admin_')) {
        ...
        } else if (data === 'admin_chats') {
            bot.sendMessage(chatId, 'Use /activechats to see active chats');
        } else if (data === 'admin_broadcast') {
            bot.sendMessage(chatId, 'Use /broadcast to send a message to all users');
        } else if (data === 'admin_broadcast_premium') {
            bot.sendMessage(chatId, 'Use /broadcastpremium to send to premium users only');
        }
        return;
    }
});
